...
The desktop requirements include the Phase 3 Pydantic schema runtime. The
smaller domain/persistence-only dependency set is available in
...
- Phase 3 `.diesel.db` engineering datasets with Pydantic validation, SQLite transactions, R-Tree spatial queries, stable IDs, immutable revisions, relationships, and append-only review decisions
- File → Migrate Legacy Project to Dataset preserves the original `.dieselpdf.json`, records its SHA-256 and raw payload, converts every Canvas object, and reports unmapped fields instead of silently dropping them
- File → Open Engineering Dataset shows database-owned geometry, semantic objects, relationships and revisions with drawing/table cross-selection
- AI and system identities may propose work but cannot create an engineer-approved state; approval requires an explicitly authorised engineer identity and a recorded reason
- Deterministic `.diesel.jsonl` export/import preserves revisions, review decisions, provenance and legacy migration evidence
...
Run the complete Phase 1–3 suite with:
