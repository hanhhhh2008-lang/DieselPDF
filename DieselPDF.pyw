...
def import_optional_dependency(module_names, vendor_dir):
    """Import an installed package first, then try the bundled Windows copy."""
    ...
    inserted_vendor_path = vendor_dir not in sys.path
...
class DieselPDF(tk.Tk):
    ...
    def show_file_menu(self):
        ...
        menu.add_command(label="Open Engineering Dataset...", command=self.open_engineering_dataset)
        menu.add_command(label="Migrate Legacy Project to Dataset...", command=self.import_legacy_to_dataset)
        menu.add_separator()
    ...
    def _populate_ribbon(self, tab):
        ...
        if tab == "Home":
            ...
            self._cmd_button(files, "Dataset", self.open_engineering_dataset)
            self._cmd_button(files, "Migrate", self.import_legacy_to_dataset)
    ...
    def _pdf_renderer(self):
        renderer, _error = import_optional_dependency(
            ("pymupdf", "fitz"),
            PDF_VENDOR_DIR,
        )
        return renderer
    ...
    def open_engineering_dataset(self, path=None):
        if path is None:
            path = filedialog.askopenfilename(
                filetypes=[
                    ("Diesel engineering dataset", "*.diesel.db"),
                    ("SQLite database", "*.db"),
                    ("All files", "*.*"),
                ]
            )
        if not path:
            return
        try:
            from dieselpdf.ui.tkinter.dataset_window import EngineeringDatasetWindow

            window = EngineeringDatasetWindow(self, path)
            window.transient(self)
        except Exception as exc:
            messagebox.showerror(
                "Engineering Dataset",
                f"Could not open the engineering dataset:\n{exc}\n\n"
                "Install Phase 3 dependencies with: pip install -r requirements-core.txt",
            )
            return
        self._set_status(f"Opened engineering dataset: {os.path.basename(path)}")

    def import_legacy_to_dataset(self):
        source_path = filedialog.askopenfilename(
            filetypes=[
                ("Legacy DieselPDF project", "*.dieselpdf.json"),
                ("JSON", "*.json"),
                ("All files", "*.*"),
            ]
        )
        if not source_path:
            return
        dataset_path = filedialog.asksaveasfilename(
            defaultextension=".diesel.db",
            filetypes=[("Diesel engineering dataset", "*.diesel.db")],
        )
        if not dataset_path:
            return
        try:
            ...
            report = LegacyProjectImporter().import_file(source_path, dataset_path)
        except Exception as exc:
            messagebox.showerror(
                "Legacy Migration",
                f"Migration was not completed:\n{exc}\n\n"
                "The original legacy project was not modified.",
            )
            return
        messagebox.showinfo(
            "Legacy Migration Complete",
            f"Imported {report.entity_count} of {report.object_count} Canvas objects.\n"
            f"Pages: {report.page_count}\n"
            f"Warnings: {len(report.warnings)}\n"
            f"Unmapped fields retained in raw import evidence: {len(report.unmapped_fields)}\n"
            f"Original preserved: {'Yes' if report.original_preserved else 'No'}",
        )
        self.open_engineering_dataset(dataset_path)
    ...
    def _cad_backend(self):
        backend, error = import_optional_dependency("ezdxf", CAD_VENDOR_DIR)
        if backend is not None:
            return backend
        messagebox.showerror(
            "CAD module",
            "CAD features require ezdxf. Run the platform setup script or install "
            f"requirements-desktop.txt.\n\n{error}",
        )
        return None
